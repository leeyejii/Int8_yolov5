...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        tensor.
        """
        return self.ff.add(x, self.cv2(self.cv1(x))) if self.add else self.cv2(self.cv1(x))#self.quant(self.dequant(x) + self.dequant(self.cv2(self.cv1(x)))) if self.add else self.cv2(self.cv1(x))
...
class BottleneckCSP(nn.Module):
    ...
    def forward(self, x):
        ...
        y2 = self.cv2(x)
        return self.cv4(self.act(self.bn(FloatFunctional().cat((y1, y2), 1))))
...
class C3(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.cv3(FloatFunctional().cat((self.m(self.cv1(x)), self.cv2(x)), 1))
...
class SPP(nn.Module):
    ...
    def forward(self, x):
        ...
        with warnings.catch_warnings():
            ...
            return self.cv2(FloatFunctional().cat([x] + [m(x) for m in self.m], 1))
...
class SPPF(nn.Module):
    ...
    def forward(self, x):
        ...
        with warnings.catch_warnings():
            ...
            y1 = self.m(x)
            y2 = self.m(y1)
            return self.cv2(FloatFunctional().cat((x, y1, y2, self.m(y2)), 1))
...
class Focus(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.conv(FloatFunctional().cat((x[..., ::2, ::2], x[..., 1::2, ::2], x[..., ::2, 1::2], x[..., 1::2, 1::2]), 1))
...
class GhostConv(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.cv1(x)
        return FloatFunctional().cat((y, self.cv2(y)), 1)
...
class Concat(nn.Module):
    ...
    def forward(self, x):
        ...
        int.
        """
        return FloatFunctional().cat(x, self.d)
...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights="yolov5s.pt", device=torch.device("cpu"), dnn=False, data=None, fp16=False, fuse=True):
        ...
        if pt:  # PyTorch
            ...
            model.float()#model.half() if fp16 else model.float()
            #model.fuse()  # Fuse Convolution, BatchNorm, ReLU layers
    ...
    def warmup(self, imgsz=(1, 3, 640, 640)):
        ...
        if any(warmup_types) and (self.device.type != "cpu" or self.triton):
            im = torch.empty(*imgsz, dtype=torch.float, device=self.device)  # input dtype=torch.half if self.fp16 else torch.float
