...
def train(hyp, opt, device, callbacks):
    ...
    if RANK in {-1, 0}:
        ...
        if not resume:
            ...
            model.float()#half().float()  # pre-reduce anchor precision
        ...
        if RANK in {-1, 0}:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ckpt = {
                    ...
                    "model": deepcopy(de_parallel(model.float())).float(),#.half(),
                    "ema": deepcopy(ema.ema).float(),#.half(),
                    "updates": ema.updates,
            ...
            if f.exists():
                ...
                if f is best:
                    ...
                    results, _, _ = validate.run(
                        ...
                        batch_size=batch_size // WORLD_SIZE * 2,
                        imgsz=imgsz,
                        model=attempt_load(f, device).float(),#.half(),
